...
public sealed class StorageCoverageTests
{
    ...
    public void AnimationInfoRetainsFramesAndTiming()
    {
        ...
        TraverseUtils.SetValue(ref animation, "<Sprites>k__BackingField", sprites);
        TraverseUtils.SetValue(ref animation, "<SecondsPerFrame>k__BackingField", 0.25f);
    ...
    {
        Assert.Throws<ArgumentException>(() => new AnimationImportInfo(null!, "path", 1, 1, 1f));
        Assert.Throws<ArgumentException>(() => new AnimationImportInfo(string.Empty, "path", 1, 1, 1f));
        Assert.Throws<ArgumentException>(() => new AnimationImportInfo("name", null!, 1, 1, 1f));
        Assert.Throws<ArgumentException>(() => new AnimationImportInfo("name", string.Empty, 1, 1, 1f));
        Assert.Throws<ArgumentException>(() => new AnimationImportInfo("name", "path", 0, 1, 1f));
        Assert.Throws<ArgumentException>(() => new AnimationImportInfo("name", "path", 1, 0, 1f));
        Assert.Throws<ArgumentException>(() => new AnimationImportInfo("name", "path", 1, 1, 0f));
    }
    ...
    public void AnimationStorageSupportsRegisterReplaceAndTryOperations()
    {
        ...
        Assert.Throws<ArgumentException>(() => storage.Register(null!));
        Assert.Throws<ArgumentException>(() => storage.Replace(null!));
    ...
    public void SpriteStorageExposesRegistrationOperations()
    {
        ...
        Assert.Throws<ArgumentException>(() => storage.Register(null!, null!));
    }
    ...
    {
        // ponytail: use an uninitialized fixture because this net8 smoke host has no Unity native runtime.
        AnimationInfo animation = (AnimationInfo)RuntimeHelpers.GetUninitializedObject(typeof(AnimationInfo));
